...
void init_timers() {
	...
	LL_DMA_ConfigAddresses(DMA2, LL_DMA_STREAM_1, (uint32_t) &VoiceBuff0[0],
			(uint32_t) &TIM1->CCR1,
	...
	LL_DMA_ConfigAddresses(DMA2, LL_DMA_STREAM_2, (uint32_t) &VoiceBuff1[0],
			(uint32_t) &TIM1->CCR2,
...
uint32_t readSpeedXScaled = 1.001 * TIME_SCALE_FACT * USBD_AUDIO_FREQ
		/ (float) FREQ;
...
struct LR getNextSampleLR() {
	...
	if (UsbSamplesAvail) {
		...
		int32_t W1 = readPositionXScaled
				>> (TIME_BIT_SCALE_FACT - BIT_SHIFT_SCALE_FACT)
				& ((1 << BIT_SHIFT_SCALE_FACT) - 1);
...
void AUDIO_OUT_Periodic(uint16_t *pBuffer, uint32_t Size) {
	...
	if (cPos == 0) {
		uint16_t timeForRecivedSamples = lastAudioUsbTimeStampNew
				- lastAudioUsbTimeStamp;

		timeForRecivedSamples_mean[pnt_timeForRecivedSamples_mean++] =
				timeForRecivedSamples;
		if (pnt_timeForRecivedSamples_mean > 2)
			pnt_timeForRecivedSamples_mean = 0;
	...
	if (cPos == 0) {
		...
		int timeForRecivedSamples = median3(timeForRecivedSamples_mean[0],
				timeForRecivedSamples_mean[1], timeForRecivedSamples_mean[2]);
		//(TIMER_CLOCK_FREQ*(float)outDmaSpeedScaled)/TIME_SCALE_FACT;
		if (timeForRecivedSamples)
			inputSpeed = samplesInBuff * TIMER_CLOCK_FREQ
					/ timeForRecivedSamples;
		...
		int approximateSamplesOutedFromLastDMA = ((float) timeFromLastDMA
				/ TIMER_CLOCK_FREQ) * inputSpeed;

		int appDistance =
				(int) (lastDmaPos + approximateSamplesOutedFromLastDMA) - cPos;
		if (appDistance < 0)
		...
		if (UsbSamplesAvail) {
			if (timeForRecivedSamples) {
				...
				if (err > samplesInBuffH / 2 || err < -samplesInBuffH / 2) //seems completely lost sync , force set frequency
						{
					float outSpeed = (TIMER_CLOCK_FREQ
							* (float) outDmaSpeedScaled) / TIME_SCALE_FACT;
					sForcedSpeed = (int) (inputSpeed * TIME_SCALE_FACT
							/ outSpeed);
		...
		prevPos = appDistance;
		UsbSamplesAvail = samplesInBuff
				* ((int) ((float) FREQ / USBD_AUDIO_FREQ + 0.5f)) * 2;
	}
...
void readDataTim(int offset) {
	...
	for (int k = 0; k < N_SIZE / 2; k++) {
		struct LR tt = getNextSampleLR(/*k+ASBUF_SIZE/4*/);
		VoiceBuff0[k + offset] = sigma_delta2_SCALED(&static_L_channel2_SCALED,
				(MAX_VOL * (tt.L + (1 << USB_DATA_BITS_H)))
						>> (USB_DATA_BITS - SIGMA_BITS));
		VoiceBuff1[k + offset] = sigma_delta2_SCALED(&static_R_channel2_SCALED,
				(MAX_VOL * (tt.R + (1 << USB_DATA_BITS_H)))
						>> (USB_DATA_BITS - SIGMA_BITS));
	}
#endif
#endif
...
void paintLevels(float level, int full) {
	...
	uint16_t tc = color_convertRGB_to16d(base_color.R, base_color.G,
			base_color.B);
#define RAMP 30
	...
	for (int colorv = lastPos - RAMP; colorv < lastPos; colorv++) {
		struct rgb base_color = hsv2rgb(REF_COLOR, 95,
				90 - offset - 1 + colorv - lastPos);
		uint16_t tc = color_convertRGB_to16d(base_color.R, base_color.G,
				base_color.B);
		ctable[colorv * 2 + 0] = (tc >> 8u) & 0xffu;
		ctable[colorv * 2 + 1] = tc & 0xffu;
	...
	if (full) {
		...
		LCD_fillRectDataTable(EOFS, EOFS, LCD_getWidth() - EOFS * 2,
				LCD_getHeight() - EOFS * 2, ftable, ctable);
	} else {
		LCD_fillRectDataTable(EOFS, EOFS, LCD_getWidth() - EOFS * 2,
				LCD_getHeight() - EOFS * 2, ftable, ctable);
	}
...
void tableSetup1() {
	...
	for (int y = EOFS; y < 240 - EOFS; y++) {
		for (int x = EOFS; x < 240 - EOFS; x++) {
			...
			if (norma
					< (240 - EOFS * 2)&&norma>35&&x>EOFS&&x<240-EOFS&&y>EOFS&&y<240-EOFS) {
				ftable[(y - EOFS) * (240 - (EOFS * 2)) + x - EOFS] = pos;
			} else {
				ftable[(y - EOFS) * (240 - (EOFS * 2)) + x - EOFS] = 0;
			}
...
int main(void) {
  ...
  HAL_Delay(100);
	...
	SYS_CLK_MHZ * 1000000.0 / (MAX_VOL), sForcedSpeed,
			HalfTransfer_CallBack_FS_Counter,
			TransferComplete_CallBack_FS_Counter, AUDIO_PeriodicTC_FS_Counter,
			readSpeedXScaled);
	int mode = 2;
#if (NUMBER_OF_LCD)
...
//	  }
	...
	while (1) {
		...
		if (HAL_GetTick() / 2048 != ko) {
			...
			printf(
					"MAX_VOL = %d INSpeed=%0.01f Hz outSpeed=%0.01f Hz %x Delta %d SMPInH %d elaps = %d mS,ForcedSpeed = %x  %04d PLLspeed = %x %01d %07d s=%d \n",
					MAX_VOL, inputSpeed,
					(TIMER_CLOCK_FREQ * (float) outDmaSpeedScaled)
							/ TIME_SCALE_FACT, outDmaSpeedScaled, appDistErr,
					samplesInBuffH, elapsed_time_ticks, sForcedSpeed,
					AUDIO_PeriodicTC_FS_Counter, readSpeedXScaled,
					HAL_GPIO_ReadPin(KEY_GPIO_Port, KEY_Pin), TIM2->CNT,
					UsbSamplesAvail);
...
static void MX_TIM1_Init(void) {
	...
	LL_DMA_SetStreamPriorityLevel(DMA2, LL_DMA_STREAM_1,
	LL_DMA_PRIORITY_HIGH);

	LL_DMA_SetMode(DMA2, LL_DMA_STREAM_1, LL_DMA_MODE_CIRCULAR);

	LL_DMA_SetPeriphIncMode(DMA2, LL_DMA_STREAM_1,
	...
	LL_DMA_SetFIFOThreshold(DMA2, LL_DMA_STREAM_1,
	...
	LL_DMA_SetPeriphIncMode(DMA2, LL_DMA_STREAM_2,
	LL_DMA_PERIPH_NOINCREMENT);
	...
	LL_DMA_SetFIFOThreshold(DMA2, LL_DMA_STREAM_2,
	LL_DMA_FIFOTHRESHOLD_FULL);
...
static void MX_GPIO_Init(void) {
	...
	MUTE_OUT_Pin | LCD_CMD_Pin | LCD_RESET_Pin | LCD_BL_Pin, GPIO_PIN_RESET);
	...
	GPIO_InitStruct.Pin = MUTE_OUT_Pin | LCD_CMD_Pin | LCD_RESET_Pin
			| LCD_BL_Pin;
	GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
	GPIO_InitStruct.Pull = GPIO_NOPULL;
	GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
